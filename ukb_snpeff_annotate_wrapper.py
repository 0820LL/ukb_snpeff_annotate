...
def steward(config_file_path:str, ukb_snpeff_annotate_path:str, send_message_script:str) -> None:
    ...
    ukb_snpeff_annotate_command = 'nextflow run -offline -profile singularity -bg -params-file {} {} >> run_log.txt'.format(params_file_path, ukb_snpeff_annotate_path)
    return_value = os.system(ukb_snpeff_annotate_command)
    with open('ukb_snpeff_annotate_command.txt', 'w') as command_f:
        command_f.write(ukb_snpeff_annotate_command + '\n')
    ...
    feedback_dict = {
        ...
        'pipelineName'    : 'ukb_snpeff_annotate',
        'analysisStatus'  : '',
                    ...
                    if 'COMPLETED' in line:
                        ...
                        for file in os.listdir('{}/results'.format(analysis_path)):
                            if file.endswith('.ann.vcf'):
                                ann_vcf = '{}/results/{}'.format(analysis_path, file)
                            if file.endswith('.csv'):
                                csv = '{}/results/{}'.format(analysis_path, file)
                            if file.endswith('.html'):
                                html = '{}/results/{}'.format(analysis_path, file)
                            if file.endswith('.genes.txt'):
                                gene_txt = '{}/results/{}'.format(analysis_path, file)
                            ...
                            'text' : [
                                {
                                    'sort'   : 1,
                                    'title'  : 'snpEff注释结果1',
                                    'content': 'snpEff注释结果文件1：#&{}'.format(ann_vcf),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                },
                                {
                                    'sort'   : 2,
                                    'title'  : 'snpEff注释结果2',
                                    'content': 'snpEff注释结果文件2：#&{}'.format(csv),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                },
                                {
                                    'sort'   : 2,
                                    'title'  : 'snpEff注释结果3',
                                    'content': 'snpEff注释结果文件3：#&{}'.format(html),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                },
                                {
                                    'sort'   : 2,
                                    'title'  : 'snpEff注释结果4',
                                    'content': 'snpEff注释结果文件4：#&{}'.format(gene_txt),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                }
...
def main() -> None:
    ...
    parser.add_argument('--ukb_snpeff_annotate_path', required=True, help='the full path of the ukb_snpeff_annotate')
    ...
    ukb_snpeff_annotate_path = args.ukb_snpeff_annotate_path
    send_message_script = args.send_message_script
    steward(config_file_path, ukb_snpeff_annotate_path, send_message_script)
